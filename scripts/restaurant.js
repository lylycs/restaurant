...
async function getRestaurant() {
	try {
		fetch("http://localhost:3000/v1/restaurant/restaurants/" + restaurantId)
			.then((r) => r.json())
			.then((r) => {
				...
				restaurantImage.src = "http://localhost:3000/" + r.displayImage;
...
async function updateRestaurant() {
	...
	try {
		fetch("http://localhost:3000/v1/restaurant/restaurant/" + restaurantId, {
			method: "PATCH",
			headers: {
...
function handleDisplayUpload() {
	...
	try {
		...
		request.open(
			"POST",
			"http://localhost:3000/v1/restaurant/restaurant/avatar"
		);
...
async function uploadImage() {
	...
	var request = new XMLHttpRequest();
	request.open("POST", "http://localhost:3000/v1/restaurant/restaurant/photos");
	request.setRequestHeader("Authorization", localStorage.getItem("token"));
	request.onload = async function (e) {
		const fresponse = await JSON.parse(request.response);
		if (request.status === 200) {
...
async function deleteImage(photoId) {
	console.log(photoId);
	try {
		fetch("http://localhost:3000/v1/restaurant/restaurant/photos/" + photoId, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				Authorization: localStorage.getItem("token"),
...
async function updateAddress() {
	...
	try {
		fetch("http://localhost:3000/v1/restaurant/address/" + restaurantId, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: localStorage.getItem("token"),
...
async function deleteAddress() {
	try {
		fetch("http://localhost:3000/v1/restaurant/address/" + address._id, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				Authorization: localStorage.getItem("token"),
...
restaurantComponent = (p) => {
	return `<img ondblclick="deleteImage('${p._id}')" src="http://localhost:3000/${p.path}" alt="${p._id}" />`;
};
