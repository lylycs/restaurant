...
async function getReviews() {
	try {
		fetch("http://localhost:3000/v1/restaurant/reviews/" + restaurantId)
			.then((r) => r.json())
			.then((r) => {
				if (r.length === 0) {
...
const reviewCard = (r) => {
	...
	<div class="review-card-header">
		<img src="http://localhost:3000/${r.userId.displayImage}" alt="profile" />
		<div>
	...
	<div class="review-card-body">
		...
		<div class="review-card-images">
			${r.medias.map(
				(m) => `<img src="http://localhost:3000/${m.path}" alt="review img" />`
			)}
		</div>
	</div>
</div>`;
